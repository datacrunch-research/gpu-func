#!/usr/bin/env python3
"""Fine-tune Nemotron 3.5 Lightning through public gfaas APIs.
...
LIGHTNING_ARCHITECTURE = "NemotronHForCausalLM"
LIGHTNING_MTP_BLOCKS = ["attention", "moe"]
LIGHTNING_MTP_ITERATIONS = 2
LIGHTNING_MTP_LOSS_SCALE = 0.1
LIGHTNING_LORA_TARGET_MODULES = [
    "*in_proj",
    "*q_proj",
    "*k_proj",
    "*v_proj",
...
LIGHTNING_ROUTED_EXPERT_LORA_TARGET = "*.experts"
LIGHTNING_LORA_RANK = 8
LIGHTNING_LORA_ALPHA = 32
...
def validate_adapter_manifest(
    ...
    *,
    ...
    """Require the model-specific Lightning LoRA shape in the final adapter."""
    config_path = adapter_root / "adapter_config.json"
    try:
        config = json.loads(config_path.read_text(encoding="utf-8"))
    except FileNotFoundError as error:
        raise RuntimeError("AutoModel did not save adapter_config.json") from error
    except (OSError, json.JSONDecodeError) as error:
        raise RuntimeError(f"AutoModel saved an invalid adapter_config.json: {error}") from error
    if not isinstance(config, dict):
        raise RuntimeError("AutoModel adapter_config.json must contain an object")

    rank = config.get("r")
    alpha = config.get("lora_alpha")
    targets = config.get("target_modules")
    if rank != LIGHTNING_LORA_RANK or alpha != LIGHTNING_LORA_ALPHA:
        raise RuntimeError(
            "AutoModel saved the wrong Lightning LoRA rank or alpha: "
            f"expected {LIGHTNING_LORA_RANK}/{LIGHTNING_LORA_ALPHA}, got {rank}/{alpha}"
        )
    if (
        ...
        raise RuntimeError("AutoModel adapter_config.json has no target module list")
    if any(target == "lm_head" or target.endswith(".lm_head") for target in targets):
        raise RuntimeError("Lightning LoRA adapter must not include lm_head")
    ...
    observed_suffixes = {target.rsplit(".", 1)[-1] for target in targets}
    missing = sorted(required_suffixes - observed_suffixes)
    if missing:
        raise RuntimeError(f"Lightning LoRA adapter is missing projection targets: {missing}")
    routed_expert_targets = [
        target
        for target in targets
        if ".experts." in target
        and target.rsplit(".", 1)[-1] in {"gate_proj", "up_proj", "down_proj"}
    ]
    if require_routed_experts and not routed_expert_targets:
        raise RuntimeError("Lightning expert-parallel LoRA adapter has no routed-expert targets")
    if not require_routed_experts and routed_expert_targets:
        raise RuntimeError(
            "Lightning DDP LoRA adapter must not include routed experts; "
            "AutoModel copies their base weights into each DDP process"
        )
    return {
        "rank": rank,
        "alpha": alpha,
        ...
        "routed_expert_target_count": len(routed_expert_targets),
    }
...
def publish_latest_checkpoint(
    ...
    """Publish the first completed checkpoint at or after each requested interval."""
    ...
    if latest is None or latest.name == "resume" or latest.name in published:
        return
    match = re.search(r"(?:^|_)step_(\d+)$", latest.name)
    if match is None:
        raise RuntimeError(f"AutoModel wrote an unrecognized checkpoint name: {latest.name!r}")
    completed_steps = int(match.group(1)) + 1
    next_publication_step = (len(published) + 1) * checkpoint_every_steps
    if completed_steps < next_publication_step:
        return
...
def resolve_global_batch_size(
    emitted_samples: int,
    gpu_count: int,
    requested_size: int,
) -> int:
    """Select a data-parallel global batch that fits the qualified corpus."""
    if emitted_samples < 1:
        raise ValueError("emitted_samples must be positive")
    if gpu_count < 1:
        raise ValueError("gpu_count must be positive")
    if requested_size < gpu_count or requested_size % gpu_count != 0:
        raise ValueError("global_batch_size must be a positive multiple of gpu_count")
    if emitted_samples < gpu_count:
        raise ValueError(
            "qualified corpus has fewer emitted samples than requested GPU ranks: "
            f"samples={emitted_samples}, gpu_count={gpu_count}"
        )
    return min(requested_size, emitted_samples - emitted_samples % gpu_count)
...
def resolve_training_steps(
    ...
    """Resolve an explicit step count or whole-corpus epoch count."""
    if emitted_samples < 1:
        raise ValueError("emitted_samples must be positive")
    if global_batch_size < 1:
        raise ValueError("global_batch_size must be positive")
    if (max_steps is None) == (epochs is None):
        raise ValueError("specify exactly one of max_steps or epochs")
    ...
    return math.ceil(emitted_samples / global_batch_size) * epochs
...
def make_recipe(
    *,
    ...
    if gpu_count < 1:
        raise ValueError("gpu_count must be positive")
    if global_batch_size < gpu_count or global_batch_size % gpu_count != 0:
        raise ValueError("global_batch_size must be a positive multiple of gpu_count")
    ...
    model = lightning_model_recipe(model_path)
    ...
    return {
        ...
        # Each data-parallel rank receives one local sample. AutoModel uses
        # gradient accumulation to reach the requested global batch.
        "step_scheduler": {
        ...
        # Select the Lightning projections explicitly. AutoModel's cookbook
        # exclude-list fallback also selects lm_head. FSDP2 uses expert
        # parallelism so each rank owns a shard of the routed experts. DDP must
        # keep them out because its GroupedExperts wrapper replicates their
        # base tensors in every process. Mamba out_proj remains excluded
        # because its custom kernel reads out_proj.weight directly.
        "peft": {
            "_target_": "nemo_automodel.components._peft.lora.PeftConfig",
            "target_modules": lightning_lora_target_modules(distributed_strategy),
            "dim": LIGHTNING_LORA_RANK,
            "alpha": LIGHTNING_LORA_ALPHA,
            "dropout_position": "pre",
            "use_triton": use_triton,
        },
        "distributed": distributed_config(distributed_strategy, gpu_count),
        ...
        # These optimizer values match NVIDIA's AutoModel Lightning cookbook.
        "optimizer": {
        ...
        "lr_scheduler": {
            ...
            # AutoModel requires warmup to end before decay. Keep NVIDIA's
            # ten-step warmup for normal runs, but shorten bounded smoke runs.
            "lr_warmup_steps": min(10, max_steps - 1),
            "min_lr": 1e-5,
        },
        "nvtx": False,
    }
...
def load_lightning_model_config(path: Path) -> dict[str, Any]:
    """Load and identify the pinned Nemotron 3.5 Lightning model format."""
    config_path = path / "config.json"
    try:
        config = json.loads(config_path.read_text(encoding="utf-8"))
    except FileNotFoundError as error:
        raise ValueError("model directory is missing config.json") from error
    except (OSError, json.JSONDecodeError) as error:
        raise ValueError(f"model config is not readable JSON: {error}") from error
    if not isinstance(config, dict):
    ...
    if config.get("architectures") != [LIGHTNING_ARCHITECTURE]:
        raise ValueError(f"model config does not identify {LIGHTNING_ARCHITECTURE}")
    if config.get("num_nextn_predict_layers") != 1:
        raise ValueError("model config does not contain the Lightning physical MTP layer")
    if config.get("mtp_layers_block_type") != LIGHTNING_MTP_BLOCKS:
        raise ValueError("model config does not contain the Lightning attention and MoE MTP blocks")
    if not isinstance(config.get("n_routed_experts"), int):
        raise ValueError("model config does not identify the Lightning routed experts")
    return config


def lightning_model_recipe(path: Path) -> dict[str, Any]:
    """Create the AutoModel section for Nemotron 3.5 Lightning."""
    load_lightning_model_config(path)
    return {
        "_target_": "nemo_automodel.NeMoAutoModelForCausalLM.from_pretrained",
        "pretrained_model_name_or_path": str(path),
        "trust_remote_code": True,
        # The checkpoint contains one physical MTP layer. NVIDIA's PEFT recipe
        # reuses that layer for predictions at t+2 and t+3.
        "num_nextn_predict_layers": LIGHTNING_MTP_ITERATIONS,
        "mtp_use_repeated_layer": True,
        "mtp_loss_scaling_factor": LIGHTNING_MTP_LOSS_SCALE,
    }
...
def validate_model_directory(path: Path) -> None:
    ...
    for required in (index_path, tokenizer_path):
...
def train_nemotron_lightning(
    ...
    reasoning_mode: str,
    tool_selection: str,
    requested_checkpoint_every_steps: int | None,
    use_triton: bool,
    ...
    global_batch_size = resolve_global_batch_size(
        qualification["emitted_samples"],
        gpu_count,
        requested_global_batch_size,
    )
    effective_max_steps = resolve_training_steps(
        qualification["emitted_samples"],
        global_batch_size,
        max_steps,
        epochs,
    ...
    checkpoint_every_steps = checkpoint_interval(
        effective_max_steps,
        requested_checkpoint_every_steps,
    )
    ...
    print(
        f"[nemotron-lora] start model={model_name} steps={effective_max_steps} "
        f"epochs={epochs if epochs is not None else 'explicit-steps'} "
    ...
    adapter_validation_error = None
    if returncode == 0 and (ADAPTER_OUTPUT.path / "adapter_model.safetensors").is_file():
        try:
            adapter_validation = validate_adapter_manifest(
                ADAPTER_OUTPUT.path,
                require_routed_experts=distributed_strategy == "fsdp2",
            )
        except RuntimeError as error:
            adapter_validation_error = str(error)
    report = {
        "model": model_name,
        "model_revision": model_revision,
        "model_artifact_id": model.artifact_id,
        ...
        "distributed_strategy": distributed_strategy,
        "mtp_iterations": LIGHTNING_MTP_ITERATIONS,
        "mtp_loss_scale": LIGHTNING_MTP_LOSS_SCALE,
    ...
    if adapter_validation_error is not None:
        raise RuntimeError(adapter_validation_error)
...
def lightning_lora_target_modules(strategy: str) -> list[str]:
    """Select projection targets that the distributed strategy can own safely."""
    if strategy == "fsdp2":
        return [*LIGHTNING_LORA_TARGET_MODULES, LIGHTNING_ROUTED_EXPERT_LORA_TARGET]
    if strategy == "ddp":
        return list(LIGHTNING_LORA_TARGET_MODULES)
    raise ValueError(f"unsupported distributed strategy: {strategy}")


def distributed_config(strategy: str, gpu_count: int) -> dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    ...
    duration.add_argument(
        "--max-steps",
        type=positive_integer,
        help="exact optimizer-step count (default: 1 when --epochs is absent)",
    )
    duration.add_argument(
        "--epochs",
        type=positive_integer,
        help="whole qualified-corpus passes; the worker computes the optimizer-step count",
    )
    parser.add_argument(
        "--global-batch-size",
        type=positive_integer,
        default=DEFAULT_GLOBAL_BATCH_SIZE,
        help="samples per optimizer step across all ranks (default: 8)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="fsdp2",
        help=("multi-GPU parameter strategy; FSDP2 also shards routed experts (default: fsdp2)"),
    )
...
def main() -> int:
    ...
    with gfaas.Client() as client:
        ...
        job = client.submit(
            ...
            function=train_nemotron_lightning,
            args=(
                model,
                training_data,
                resume_checkpoint,
            ...
            app_name="nemotron-lightning-lora",
    ...
    local_report = {
        ...
        "requested_global_batch_size": args.global_batch_size,
        "requested_checkpoint_every_steps": args.checkpoint_every_steps,
        "triton": args.triton,
